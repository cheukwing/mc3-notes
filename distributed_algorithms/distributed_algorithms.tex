...
\begin{itemize}
  \item \textbf{No Duplication (S)} and \textbf{No Creation (S)} derived from PL.
...
When a message is received from a crashed process then the message is broadcast, otherwise if it is a correct process it is not.
...
The vector clock (an array of sequence numbers) indicates the \textbf{number of messages from each process that have been delivered}.
...
Processes consist of an initial state, a set of states, message generation function, and state transition function.
...
\begin{itemize}
  ...
  \item \textbf{Uniform Agreement} - follows from the uniform agreement property of the underlying uniform consensus abstraction.
...
\subsubsection{Strict Consistency/Linearisability}
\begin{itemize}
...
We want heterogeneous nodes in terms of resource capacity; all nodes are equal and communicate directly (no client-server relationship); and high churn with the entering and leaving of nodes.
